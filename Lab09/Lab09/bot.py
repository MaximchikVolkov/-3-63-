...
def start(msg):
    ...
    keyboard.row(*[telebot.types.InlineKeyboardButton(text=name, callback_data=name) for name in ['Проголосовать']])
    keyboard.row(*[telebot.types.InlineKeyboardButton(text=name, callback_data=name) for name in ['Посмотреть результаты']])
...
def result(msg):
    if msg.data == 'Проголосовать':
        keyboard(msg)
    elif msg.data == 'Посмотреть результаты':
